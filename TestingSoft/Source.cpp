...
public:
	...
	{
			system("cls");	SoftNameShow(); VectGuests.clear();
		cout << "---------------------------------------------------------" << endl;
...
//--------------------------------------------------------------------------------
			...
			if ((temp_coice - '0' >= 0) && (temp_coice - '0' < size_of_array))
			{
				...
				else
				{
					...
					for (int i = 0; i < VectTests.size(); i++)//âûâîä íà ıêğàí òåñòîâ äëÿ îïğåäåëåíèÿ êàêîé òåñò ïğîéòè
					{
						cout << "\nÍÎÌÅĞ ÒÅÑÒÀ " << i << ":" << endl;
						cout << "-------------------------------------------------------------------" << endl;
						VectTests.at(i).DisplayTestForPassing();
					}
					cout << "ÓÊÀÆÈÒÅ ÍÎÌÅĞ ÒÅÑÒÀ ÏĞÎÕÎÆÄÅÍÈß :" << endl;
					char temp_choice_answ;
					for (;;) {
						cin >> temp_choice_answ;
						if ((temp_choice_answ - '0' >= 0) && (temp_choice_answ - '0' < VectTests.size()))
							break;
						else
							cout << "ÑÄÅËÀÉÒÅ ÏĞÀÂÈËÜÍÛÉ ÂÛÁÎĞ!" << endl;
					}
					...
					VectTests.at(temp_choice_answ - '0').DisplayTestForPassing();// test in work
					...
					for (int i = 0; i < 4; i++)
					{
						if (!strcmp(VectTests.at(temp_choice_answ - '0').GetQuestArr()[0].answers_arr[i],
							VectTests.at(temp_choice_answ - '0').temp_answ_arr1[temp_answ1 - '0' - 1]	)	)
							temp_c = i; //break;
					}
					
					if (VectTests.at(temp_choice_answ - '0').GetQuestArr()[0].correct_answ_arr[temp_c] == true )
					{
						cout << "ÂÅĞÍÎ!" << endl;
						counter_corr_answ++;	
					}
					...
					for (int i = 0; i < 4; i++)
					{
						if (!strcmp(VectTests.at(temp_choice_answ - '0').GetQuestArr()[1].answers_arr[i],
							VectTests.at(temp_choice_answ - '0').temp_answ_arr2[temp_answ2 - '0' - 1]))
							temp_c = i; //break;
					}

					if (VectTests.at(temp_choice_answ - '0').GetQuestArr()[1].correct_answ_arr[temp_c] == true)
					{
						cout << "ÂÅĞÍÎ!" << endl;
						counter_corr_answ++;
					}
					...
					cout << "-------------------------------------------" << endl;
				char login_coding[50]; strcpy(login_coding, guest_login_stat);
					
				strcpy(my_test_stat->stat_guest_login, (char *)CodingStr(login_coding).c_str());//çàïèñü äàííûõ ïî ïğîéäåííîìó òåñòó â îáúåêò Ñòàòèñòèêà
				strcpy(my_test_stat->stat_subject_name, subjectslab[temp_coice - '0']);
				strcpy(my_test_stat->stat_test_name, VectTests.at(temp_choice_answ - '0').GetTestName());
		...
		else
		{
			...
			if ((temp_coice - '0' >= 0) && (temp_coice - '0' < size_of_array))
			{
				...
				else
				{
					...
					file_read.close();
					//åñëè ïîëå îáúåêòà ñòàòèñòèêè - ïğåäìåò ñîâïàäàåò ñ âûáğàííûì ïğåäìåòîì, äîáàâëÿåì îáúåêò â âåêòîğ
					for (int i = 0; i < size_of_array; i++)
					{
					...
					if (StatVectAdmin.size())
					{
						...
						cout << aver_rate_counter / StatVectAdmin.size() << " ÁÀËËÎÂ." << endl;
						cout << "-------------------------------------------------------------------" << endl;
					}
					else
					{
					...
					Sleep(10000); StatVectAdmin.clear();
				}
			}
		}
	}
		...
		for (int i = 0; i < VectTests.size(); i++)
		{
			...
			if ((temp_coice - '0' >= 0) && (temp_coice - '0' < VectTests.size()))
			{
				...
				else
				{
					...
					for (int i = 0; i < size_of_array; i++)//îòáèğàåì ñòàòèñòèêó ïî îïğåäåëåííîìó òåñòó
					{	
						if (!strcmp((p+i)->stat_test_name, VectTests.at(temp_coice - '0').GetTestName()))
						{
							StatVectAdmin.push_back(p[i]); tests_counter++;
						}
					}
					delete[]p;
					...
					Sleep(10000);VectTests.clear();StatVectAdmin.clear();VectTempLogins.clear();
				}
			}
		}
	...
	void ShowStatisticForAdminbyGuest()//âûâîä ñòàòèñòèêè ïî îïğåäåëåííîìó ïîëüçîâàòåëş
	{
		...
		cout << "\nÓÊÀÆÈÒÅ ÏÎĞßÄÊÎÂÛÉ ¹ ÏÎËÜÇÎÂÀÒÅËß ÄËß ÂÛÂÎÄÀ ÑÒÀÒÈÑÒÈÊÈ:" << endl;
		cout << "ÄËß ÂÛÕÎÄÀ Â ÏĞÅÄÛÄÓÙÅÅ ÌÅÍŞ - n : ";
		for (;;) {
			cin >> temp_coice;cin.ignore(100, '\n');
		...
		if ((temp_coice - '0' >= 0) && (temp_coice - '0' < VectGuests.size()))
		{
			...
			else
			{
				...
				for (int i = 0; i < size_of_array; i++)
				{
					if (!strcmp((p + i)->stat_guest_login, VectGuests.at(temp_coice - '0').GetGuestLogin()))
					{
						StatVectAdmin.push_back(p[i]); tests_counter++;
					}
				}
				...
				for (int i = 0; i<StatVectAdmin.size(); i++)
				{
					...
					for (int j = StatVectAdmin.size() - 1; j > i; j--)
						{
							if (!strcmp(StatVectAdmin.at(i).stat_subject_name, StatVectAdmin.at(j).stat_subject_name))
							{	flag = true;}
						}
						if (!flag)
						{VectTempSubj.push_back(StatVectAdmin.at(i).stat_subject_name);	}
				}
				...
				for (int i = 0; i < VectTempSubj.size(); i++)
				{
					...
					for (int j = 0; j < StatVectAdmin.size(); j++)
					{
						if (!strcmp(VectTempSubj.at(i).c_str(), StatVectAdmin.at(j).stat_subject_name) &&
							!strcmp(StatVectAdmin.at(j).stat_guest_login, VectGuests.at(temp_coice - '0').GetGuestLogin()))
						{
							tests_counter++;
					...
					cout << avar_rate / tests_counter << " ÁÀËËÎÂ" << endl;
				}
								
				cout << "-------------------------------------------------------------------" << endl;

				Sleep(10000);VectGuests.clear(); VectTempSubj.clear();StatVectAdmin.clear();
			}
		}
	...
	void ExportStatisticToFile()//ıêñïîğò ñòàòèñòèêè â ôàéë
	{
		...
		if ((temp_coice - '0' >= 0) && (temp_coice - '0' < VectGuests.size()))
		{
			...
			else
			{
				...
				for (int i = 0; i < size_of_array; i++)// çàïîëíÿåì âåêòîğ îáúåêòàìè êëàññà Ñòàòèñòèêà
				{
					if (!strcmp((p + i)->stat_guest_login, VectGuests.at(temp_coice - '0').GetGuestLogin()))
					{
						StatVectAdmin.push_back(p[i]); tests_counter++;
				...
				for (int i = 0; i < VectTempSubj.size(); i++)
				{
					...
					file_write << avar_rate / tests_counter << " ÁÀËËÎÂ" << endl;
				}
				file_write << "-------------------------------------------------------------------------------------------" << endl;
				...
				Sleep(5000);
				VectGuests.clear(); VectTempSubj.clear();StatVectAdmin.clear();
			}
		}
	...
	void EditGuestAcc()//êîğğåêòèğîâêà àêêàóíòà ïîëüçîâàòåëÿ
	{
		...
		system("cls");	SoftNameShow();
		cout << "---------------------------------------------------------" << endl;
		cout << "ÈÇÌÅÍÅÍÈÅ ÄÀÍÍÛÕ ÏÎ ÏÎËÜÇÎÂÀÒÅËŞ..." << endl;
		cout << "---------------------------------------------------------" << endl;
	...
	void ImportCatFromFile()// Èìïîğò êàòåãîğèé / ïğåäìåòîâ èç ğàíåå ïîäãîòâîëåííîãî ôàéëà
	{
		...
		TempVect.clear(); TempVectWithoutDupl.clear();
	}
	...
	void ImportTestFromFile()//èìïîğò òåñòà èç ğàíåå ïîäãîòîâëåííîãî ôàéëà
	{
		...
		else
		{
			...
			{
				file_read.close();
				cout << "Â ÍÀËÈÈ×ÈÈ ÅÙÅ ÍÅÒ ÒÅÑÒÎÂ, ÑÎÇÄÀÉÒÅ ÏÅĞÂÛÉ ÒÅÑÒ ×ÅĞÅÇ ÏĞÎÃĞÀÌÌÓ!..." << endl;
			}
			else
			{
				file_read.seekg(0, ios::end);
				...
				cout << "ÓÊÀÆÈÒÅ ÈÌß ÔÀÉËÀ ÄËß ÈÌÏÎĞÒÀ ÒÅÑÒÀ.. " << endl;
				cout << "ÍÀÏĞÈÌÅĞ, C:\\Tests.txt " << endl;
				cout << "ÒÅÑÒ ÄÎËÆÅÍ ÁÛÒÜ Â .ÒÕÒ ÔÀÉËÅ, ÊÀÆÄÛÉ ÂÎÏĞÎÑ È ÎÒÂÅÒ Ñ ÍÎÂÎÉ ÑÒĞÎÊÈ.. :" << endl;
				gets_s(temp);
						
				ifstream import(temp, ios::in);
				import.seekg(0);
				if (!import)
				...
				mytestobj->SetTestName(test_name_temp);
				...
				cout << "\nÒÅÑÒ ÓÑÏÅØÍÎ ÈÌÏÎĞÒÈĞÎÂÀÍ!" << endl;
